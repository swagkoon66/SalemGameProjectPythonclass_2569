...
class GameWindow:
    ...
    def _build_ui(self):
        ...
        is_constable = self.role == "Constable"
        if is_witch:
        ...
        self.players_canvas = tk.Canvas(pf, bg="#1e2a38", highlightthickness=0)
        ...
        self.accuse_btn  = ttk.Button(btn_row, text="🙏 Confess",    command=self._confess,     state="disabled", width=14)
    ...
    def _refresh_players(self):
        ...
        for i, p in enumerate(players):
            ...
            if not alive:
                ...
                icon = "  "
            c.create_text(PAD_X + 14, y + 14, text=icon, font=("Arial", 11), anchor="center")
    ...
    def _play_selected_card(self, event=None):
        ...
        one_target_cards = ["Accusation", "Evidence", "Testimony", "Asylum", "Piety", "Alibi", "Curse"]
    ...
    def _confess(self):
        ...
        tk.Label(dlg,
                 ...
                 bg="#2c3e50", fg="white", font=("Arial", 10), justify="center").pack(pady=6)
    ...
    def _show_dawn_dialog(self):
        ...
        tk.Label(dlg, text="As a Witch, secretly choose who receives\nthe Black Cat 🐈 (your Conspiracy target).",
                 bg="#1a1535", fg="white", font=("Arial", 11), justify="center").pack(pady=4)

        target_var = tk.StringVar()
        ttk.Combobox(dlg, textvariable=target_var,
                     values=all_players, width=24).pack(pady=12)
        ...
        ttk.Button(dlg, text="🐈 Place Black Cat", command=confirm, width=20).pack(pady=14)
    ...
    def _show_conspiracy_popup(self, transfers):
        ...
        tk.Label(dlg, text="🕯️  CONSPIRACY  🕯️",
                 bg="#1a1a2e", fg="#e67e22", font=("Arial", 15, "bold")).pack(pady=12, padx=20)
        tk.Label(dlg, text="Tryal cards have secretly changed hands!",
                 bg="#1a1a2e", fg="#bdc3c7", font=("Arial", 10)).pack(pady=2)
        ...
        for t in transfers:
            ...
            tk.Label(row, text=" ──▶ ", bg="#232d3f",
                     fg="#e67e22", font=("Arial", 10)).pack(side="left")
            ...
            if card_kind:
                ...
                tk.Label(row, text=f"{icon} {card_kind}", bg="#232d3f",
                         fg=color, font=("Arial", 9, "bold")).pack(side="left", padx=(4, 8))
            elif is_me_giver:
                tk.Label(row, text="(you gave a hidden card)", bg="#232d3f",
                         fg="#7f8c8d", font=("Arial", 9, "italic")).pack(side="left", padx=(4, 8))
            else:
        ...
        if own_tryals:
            ...
            for i, tc in enumerate(own_tryals):
                ...
                tk.Label(cards_frame, text=f"  #{i+1}  {label}",
                         bg="#1a1a2e", fg=fg, font=("Arial", 10),
                         anchor="w").pack(fill="x")

        tk.Frame(dlg, bg="#334", height=1).pack(fill="x", padx=16, pady=8)
        ttk.Button(dlg, text="Understood", command=dlg.destroy).pack(pady=10)

        dlg.update_idletasks()
        dlg.geometry(f"+{self.win.winfo_rootx()+100}+{self.win.winfo_rooty()+80}")
    ...
    def show_confess_phase(self, seconds):
        ...
        tk.Label(dlg, text="🕯️  CONFESS PHASE  🕯️",
                 bg="#1a1a2e", fg="#e67e22", font=("Arial", 15, "bold")).pack(pady=12)
        tk.Label(dlg,
                 ...
                 bg="#1a1a2e", fg="#bdc3c7", font=("Arial", 10), justify="center").pack(pady=6)
