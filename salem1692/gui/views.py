...
class GameView(ttk.Frame):
    ...
    def update_display(self):
        ...
        self.current_player_label.config(text=f"Current Player: {current}")
        
        # Update players list
        self.players_listbox.delete(0, tk.END)
        for player in state['players']:
            status = "✓" if player.alive else "✗"
            role_hint = " (W)" if player.is_witch() and player.alive else ""
            self.players_listbox.insert(tk.END, f"{status} {player.name}{role_hint}")
        ...
        if state['winner']:
            ...
            self.app.end_game(state['winner'], state['players'], duration)
            return
        
        # Check if in accusation phase
        if state['accusation_phase']:
            self.accuse_btn.config(state='disabled')
            self.draw_btn.config(state='disabled')
            
            # Show voting dialog if not already showing
            if not hasattr(self, 'voting_dialog'):
                self.show_voting_dialog(state['accused_player'])
        else:
    ...
    def show_accuse_dialog(self):
        ...
        accusable = [p.name for p in state['players'] 
                    if p.alive and p.name != current]
    ...
    def show_voting_dialog(self, accused_name):
        ...
        for player in state['players']:
            if player.alive and player.name != accused_name:
                frame = ttk.Frame(self.voting_dialog)
                frame.pack(pady=2)
                
                ttk.Label(frame, text=f"{player.name}:").pack(side='left', padx=5)
                
                def make_vote_func(voter_name):
                    def vote_func():
                        ...
                        success, msg = self.game.cast_vote(voter_name, accused_name)
                        if success:
                            self.votes_cast.add(voter_name)
                            messagebox.showinfo("Vote Cast", 
                                              f"{voter_name} votes GUILTY!")
                            
                            # Check if all votes are in
                            if len(self.votes_cast) == len(state['players']) - 1:
                ...
                ttk.Button(frame, text="Vote Guilty", 
                          command=make_vote_func(player.name)).pack(side='left', padx=5)
    ...
    def resolve_accusation(self):
        ...
        if success:
            if hasattr(self, 'voting_dialog'):
                self.voting_dialog.destroy()
                delattr(self, 'voting_dialog')
